...
public class ActivemqPluginTest {
	...
	public void testIsClientToIgnore() throws Exception {
		...
		final List<Pattern> patterns = Arrays.asList(ActivemqPlugin.JMS_CLIENT_ID_KALINKA_PUB_REGEX.split(",")).stream().filter(s -> !s.trim().isEmpty())
		...
		assertTrue(ActivemqPlugin.isClientToIgnore("kalinka-sub-456", patterns));
		assertFalse(ActivemqPlugin.isClientToIgnore("kalinka-bla-789", patterns));
	}
