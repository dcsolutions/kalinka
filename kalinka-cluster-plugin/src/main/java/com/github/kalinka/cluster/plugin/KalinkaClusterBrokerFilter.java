...
package com.github.kalinka.cluster.plugin;
 ...
 *
 */
public class KalinkaClusterBrokerFilter extends BrokerFilter {

	private final IConnectionStore connectionStore;
	private final IIdResolver idResolver;

	public KalinkaClusterBrokerFilter(final Broker next, final IConnectionStore connectionStore, final IIdResolver idResolver) {

		super(Preconditions.checkNotNull(next));
	...
	public void addConnection(final ConnectionContext context, final ConnectionInfo info) throws Exception {
		...
		final Optional<String> idOpt = this.idResolver.resolveId(context, info);
		if (idOpt.isPresent()) {
			this.connectionStore.upsertConnection(idOpt.get());
		}
